...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    column(width = 9,
           fluidRow(
             div(class = "headerbar", p(HTML("<strong>Pre</strong>Reg: Scoping Review Template")))
           ),
           fluidRow(
             div(class = "whitebar")
           ))
  ...
  fluidRow(
    column(width = 3, class = "column",
           selectInput("selecttemplate", "",
                       choices = list('Instructions'= 0, 'Template'= 1), selected = '0')
    )),
  ...
  fluidRow(
    ...
    conditionalPanel(
      condition = "input.selecttemplate == '1'",
      fluidRow(
        #  class = "row",
        column(width = 3, class = "column",
...
server <- function(input, output, session) {
  ...
  # dc.type:preregistration AND scoping review
  # "https://www.psycharchives.org/en/browse/?q=dc.type%3Apreregistration+AND+scoping+review"
  observeEvent(input$browse, {
    session$sendCustomMessage("openNewWindow", list(url = "https://www.psycharchives.org/en/browse/?q=dc.type%3Apreregistration+AND+scoping+review"))
  })
  ##### END browse link 
  ...
  observeEvent(input$uploadFile, {
    ...
    if (!is.null(inFile)) { #Datei darf nicht leer sein
      ...
      # overwrite or create scope_items with new data
